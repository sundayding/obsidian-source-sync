...
- 升级后新同步的笔记会在平台已有响应包含数据时，自动写入点赞数、收藏数和平台评论总数，不额外增加请求。
- 历史笔记可在“设置 → 同步内容 → 互动数据”中主动刷新；任务按安全节奏分轮执行，支持暂停和继续。
- 刷新“内容总览”后，表格视图会显示 `likeCount（点赞数）`、`collectCount（收藏数）` 和 `commentCount（平台评论总数）`；旧笔记缺失的数据保持为空。
